...
export default function Chessboard() {
  ...
  function selectPiece(row:number, col:number) {
    if (positions) {
      console.log(positions[row][col].name);
      isTouched = true;
    }
      ...
      squares.push(
        <View key={`${row}-${col}`} style={[styles.square, { backgroundColor, width: squareSize, height: squareSize }]}>
          <TouchableOpacity onPress={() => { selectPiece(row, col) }}>
            {positions && isTouched ? <Image source={positions[row][col].src} style={{ width: 40, height: 40, backgroundColor: 'red' }} /> : <Image source={positions[row][col].src} style={{ width: 40, height: 40 }} />}
          </TouchableOpacity>
        </View>
      );
    }
